...
using namespace life::core;

namespace {

struct Options {
    Extent size{1000, 1000};
    double density = 0.25;
    unsigned threads = 0;   // at most this many threads; 0 = hardware concurrency
    unsigned bands = 0;     // exactly this many bands (Banded only); 0 = suggestedBandCount()
    int generations = 100;
...
constexpr std::string_view kUsage =
    "usage: wxlife_bench [--size WxH] [--density 0..1] [--threads N] [--bands N] [--generations G]\n"
    "                    [--engine {0}] [--rule B3/S23] [--bounded]\n"
    "  --threads N  at most N threads (0 = all); one band per {1} cells, or 1 band below 4 of them\n"
...
enum class Parsed { Run, Help, Error };

// Reads the command line into `options`. Prints the usage for --help, and the problem for a bad argument.
...
unsigned bandCount(const Options& options)
{
    ...
    case StepperKind::Banded: {
        ...
        return std::min(bands, static_cast<unsigned>(options.size.height));   // forEachBand() caps it too
    }
...
{
    Options options;
    const Parsed parsed = parseArguments(std::span(argv, static_cast<std::size_t>(argc)).subspan(1), options);
    ...
    try {
        ...
        std::println("result  {} generations in {:.1f} ms, population {}", options.generations, total.count(),
                     formatCount(static_cast<std::uint64_t>(world.population())));
