// Smoke test of the whole wx user interface. Each test creates a real MainFrame for a World it owns, shows
// it, and drives it the way a user would:
...
// The checks read the World and what the window shows: menus, panel controls, the status bar and the
// canvas scrollbars. Modal dialogs are answered by a wxModalDialogHook, so none is ever shown.
//
// Needs a display (X11, Wayland or GTK's Broadway). Without one every test is skipped; a display that is
...
// GTK's setter for the text of an entry, such as a number box. Declared here because the tests do not see
// GTK's headers (wx/defs.h declares GtkWidget).
extern "C" void gtk_entry_set_text(GtkWidget* entry, const char* text);

namespace life::core {
// Lets GoogleTest print cell positions in failure messages. This program has no other printer to clash with.
void PrintTo(CellPos cell, std::ostream* out)
{
    *out << '(' << cell.x << ", " << cell.y << ')';
}
}
...
void recordAssertion(const wxString& file, int line, const wxString& function, const wxString& condition,
                     const wxString& message)
{
    problems().push_back("wx assertion '" + toUtf8(condition) + "' failed in " + toUtf8(function) + " (" +
                         toUtf8(file) + ":" + std::to_string(line) + "): " + toUtf8(message));
}
...
// Runs the wx event loop, as the application's main loop would (timers, paints, deleting closed frames),
// until `done()` holds or `timeout` has passed. Returns done().
...
// Whether the display draws windows at the moment. A locked or switched-off screen may draw nothing, or only
// a window's first frame; GTK then repaints no window, whatever the application does. Checked with a plain
// window that has to be painted twice.
bool displayDrawsWindows()
{
    auto* probe = new wxFrame(nullptr, wxID_ANY, "wxlife_gui_tests probe", wxDefaultPosition, wxSize(96, 96));
...
// Initialises wx as wxEntry() does, but runs no main loop: the tests run the event loop themselves, for as
// long as each step needs (see runUntil()).
class WxSession final : public testing::Environment {
public:
    static bool active() { return active_; }
    static bool startFailed() { return startFailed_; }   ///< A display is configured, but wx did not start.
    ...
    void SetUp() override
    {
        if (!displayConfigured())
            return;                                // every test skips itself
        setenv("GTK_OVERLAY_SCROLLING", "0", 0);   // as LifeApp::Initialize() does, unless the user set it
        wxApp::SetInstance(new wxApp);             // deleted by wxEntryCleanup()
        char name[] = "wxlife_gui_tests";
        char* argv[] = {name, nullptr};
        int argc = 1;
        // No ASSERT here: GoogleTest would skip every test after a fatal failure in an environment. The
        // tests report the failure instead (GuiSmokeTest::SetUp()).
        ...
        if (!recheckDrawing())
            std::fprintf(stderr, "Note: the display draws no windows (screen locked or off?), "
                                 "so paints are not checked.\n");
    }
    ...
    void TearDown() override
    {
        if (!active_)
            return;
        wxTheApp->OnExit();
        wxEntryCleanup();   // destroys any window still left, then the app
        active_ = false;
    }
...
// ---------------------------------------------------------------------------------------------------------
// User input. As with real input, a control's value changes first, then the control sends its own event.
// ---------------------------------------------------------------------------------------------------------
...
// Types `text` into a number box, replacing its contents. GTK filters it as it filters typing, and wx sends
// wxEVT_TEXT.
...
// A key event of `type` (key down, char or char hook). `modifiers` combines wxMOD_SHIFT, wxMOD_CONTROL and
// wxMOD_ALT; wx reports AltGr as wxMOD_ALTGR, which is Ctrl+Alt.
...
// Answers modal dialogs instead of showing them. wx calls Enter() before it shows a modal dialog, and
// ShowModal() returns any answer other than wxID_NONE at once.
...
public:
    ...
    std::vector<std::string> sizeTexts;   ///< The texts the last "World Size" showed after the typing.
...
protected:
    int Enter(wxDialog* dialog) override
    {
        ...
        if (!worldSize)
            return wxID_CANCEL;
        // Like a user: type both sides (width first), set the check box, and press Enter. wxGTK then presses
        // OK even while it is disabled, and wx accepts the dialog only if Validate() holds.
...
protected:
    enum StatusField { kState, kGeneration, kPopulation, kSpeed, kWorld, kView };
    ...
    void SetUp() override
    {
        ...
        if (!WxSession::active())
            GTEST_SKIP() << "No display: DISPLAY, WAYLAND_DISPLAY and BROADWAY_DISPLAY are all unset.";
        ...
        // Handlers bound later run first, so this counts every paint and Skip() lets the canvas draw it.
        canvas_->Bind(wxEVT_PAINT, [this](wxPaintEvent& event) {
            ++paints_;
            event.Skip();
        });
        frame_->ShowWithoutActivating();   // keys pressed by the person at the desktop go where they did
        paintedSince(0);
    }
    ...
    void TearDown() override
    {
        ...
        if (frame_) {
            frame_->Close();
            EXPECT_TRUE(runUntil([this] { return !frame_; })) << "The closed frame was not destroyed.";
    ...
    [[nodiscard]] wxMenuItem& menuItem(CommandId id) const { return *frame_->GetMenuBar()->FindItem(id); }
    ...
    {
        return toUtf8(frame_->GetStatusBar()->GetStatusText(field));
    }
    ...
    {
        core::CellPos cell;
        if (std::sscanf(status(kView).c_str(), "(%d, %d)", &cell.x, &cell.y) != 2)
            return std::nullopt;
        return cell;
    }

    // Moves the pointer to `at` and returns hoveredCell(). During a drag this paints or pans, like a real
    // mouse move.
    ...
    // Waits until the canvas has been painted more than `before` times. Returns false, without failing,
    // while the display draws no windows at all.
    bool paintedSince(int before)
    {
    ...
    // Asks for a repaint and waits for it, as paintedSince(). Update() paints at once where the backend
    // allows it (X11).
    bool repaint()
    {
    ...
    // The cell size that shows the whole world on the current canvas (render::Viewport::fitWorld()).
...
TEST_F(GuiSmokeTest, OpensPausedWithARandomWorldFitted)
{
    ...
    EXPECT_NEAR(static_cast<double>(world_.population()) / static_cast<double>(world_.extent().cellCount()),
                0.25, 0.01);
    ...
    // The world is fitted to the canvas. The canvas notices a late change of its size or display scale
    // when it paints, so this is checked after a paint.
    if (repaint()) {
...
TEST_F(GuiSmokeTest, RunsPausesAndSteps)
{
    ...
    EXPECT_TRUE(runUntil([&] { return std::regex_match(status(kSpeed), measured); })) << status(kSpeed);
    command(ID_RUN_PAUSE);
    const double seconds = std::chrono::duration<double>(Clock::now() - started).count();

    // The pacer never runs ahead of the target rate, and the ticks repaint the canvas (several paints, not
    // just one that was still pending).
...
TEST_F(GuiSmokeTest, ClearsRandomizesAndDrawsWithTheMouse)
{
    ...
    wxSpinCtrl& density = first<wxSpinCtrl>(group("Simulation"));
    ...
    EXPECT_EQ(world_.at(*startCell), core::kAlive);
    ...
    const core::CellCount length =
        std::max(std::abs(endCell->x - startCell->x), std::abs(endCell->y - startCell->y)) + 1;
    EXPECT_GT(length, 2);
    EXPECT_EQ(world_.population(), length);
    EXPECT_EQ(world_.at(*endCell), core::kAlive);
    ...
    mouse(*canvas_, wxEVT_LEFT_UP, end);
    EXPECT_EQ(world_.at(*endCell), core::kDead);
    ...
    // Only the button that started a drag ends it: a right click during a left drag changes nothing.
    command(ID_CLEAR);
    ...
    mouse(*canvas_, wxEVT_LEFT_UP, end);
    ASSERT_TRUE(from && to);
    EXPECT_EQ(world_.population(), std::max(std::abs(to->x - from->x), std::abs(to->y - from->y)) + 1);

    // When the camera moves during a stroke, the stroke goes on from the cell now under the pointer, with no
    // line across the jump.
    command(ID_CLEAR);
    ...
    EXPECT_GT(std::abs(jumped->x - pressed->x), 1);
    EXPECT_EQ(world_.population(), 2);
    EXPECT_EQ(world_.at(*jumped), core::kAlive);
}
...
TEST_F(GuiSmokeTest, ChangesSpeed)
{
    ...
    // The canvas matches these keys by the typed character. On a German layout ']' is AltGr+9, whose
    // key-down event says '9'.
    ...
    wxSpinCtrl& rate = first<wxSpinCtrl>(group("Speed"));
    wxSlider& slider = first<wxSlider>(group("Speed"));
    type(rate, 1000);
    ...
    EXPECT_TRUE(runUntil([&] { return std::regex_match(status(kSpeed), measuredMax); })) << status(kSpeed);
    command(ID_RUN_PAUSE);
    EXPECT_EQ(world_.population(), countedPopulation());
    ...
    // The frame takes Ctrl+M before any control sees it; in the rule box wxGTK would turn it into Enter.
    wxTextCtrl& ruleText = first<wxTextCtrl>(group("Rule"));
...
{
    wxSpinCtrl& cellSize = first<wxSpinCtrl>(group("View"));
    wxSlider& zoomSlider = first<wxSlider>(group("View"));
    ...
    ASSERT_TRUE(middle);
    EXPECT_NEAR(middle->x, 256, 1);
    EXPECT_NEAR(middle->y, 256, 1);
    for (const int orientation : {wxHORIZONTAL, wxVERTICAL}) {
        ...
        EXPECT_EQ(canvas_->GetScrollPos(orientation), (range - canvas_->GetScrollThumb(orientation)) / 2);
    }
    ...
    EXPECT_EQ(canvas_->GetScrollPos(wxHORIZONTAL), 1000 + canvas_->GetScrollThumb(wxHORIZONTAL) / 10);
    pressKey(*canvas_, WXK_PAGEUP);
    EXPECT_LT(canvas_->GetScrollPos(wxVERTICAL), end);

    // Ctrl+Home centres, handled by the canvas itself: GTK's scrolled window would take it from the menu.
    pressKey(*canvas_, WXK_HOME, wxMOD_CONTROL);
    for (const int orientation : {wxHORIZONTAL, wxVERTICAL}) {
        const int centred = (canvas_->GetScrollRange(orientation) - canvas_->GetScrollThumb(orientation)) / 2;
...
TEST_F(GuiSmokeTest, TogglesGridLinesAndWrapping)
{
    ...
    wxCheckBox& gridLines = labelled<wxCheckBox>(*panel_, "Grid lines");
    ...
    // A theme-change event keeps the grid setting and leaves the colours of the current theme. (The desktop
    // theme itself does not change here, so this cannot tell whether the colours are picked again.)
    ...
    wxCheckBox& wrap = labelled<wxCheckBox>(*panel_, "Wrap edges");
...
{
    wxChoice& presets = first<wxChoice>(group("Rule"));
    wxTextCtrl& ruleText = first<wxTextCtrl>(group("Rule"));
    ...
    // A rule without a preset, applied with the button, shows "Custom"; choosing "Custom" does nothing.
    ...
    // Invalid text leaves the rule alone, stays in the box for fixing, and the error line explains it.
    struct BadRule {
        std::string_view text;
        core::RuleError error;
    };
    const std::array badRules{BadRule{"B9/S23", core::RuleError::NeighbourOutOfRange},
                              BadRule{"Life", core::RuleError::Syntax},
                              BadRule{"   ", core::RuleError::Empty}};
    wxStaticText& errorLine = first<wxStaticText>(group("Rule"));   // the group's only text
    ...
    // Apply keeps the focus in the rule box. Other panel buttons and check boxes hand it to the canvas:
    // GTK leaves it on the clicked control, and Space would then press that control again.
    click(labelled<wxButton>(*panel_, "Apply"));
    EXPECT_EQ(wxWindow::FindFocus(), &ruleText);
...
TEST_F(GuiSmokeTest, ResizesTheWorldThroughTheSizeDialog)
{
    ...
    // Cancel changes nothing, and neither does an invalid size: the dialog explains it, disables OK and
    // refuses Enter. The typed text counts, not the value a number box would clamp it to.
    answers.worldSize.reset();
    command(ID_WORLD_SIZE);
    const core::Extent largest{core::kMaxWorldSide, core::kMaxWorldSide};
    ...
    // The smallest world, without the pattern: empty, generation 0, and fitted at the largest cell size.
    // The status bar shows the one cell, now under the resting pointer.
    command(ID_RUN_PAUSE);
    ...
    // Reference is allowed again. On a 1 × 1 torus the cell is its own eight neighbours, so it dies.
    command(ID_ENGINE_REFERENCE);
    EXPECT_EQ(world_.stepper().kind(), core::StepperKind::Reference);
    command(ID_STEP);
    EXPECT_EQ(world_.population(), 0);
...
TEST_F(GuiSmokeTest, QuitsCleanlyInTheMiddleOfAStroke)
{
    ...
    // File → Quit. The frame lets go of the mouse at once; wx deletes the frame when the loop is idle.
...
int main(int argc, char** argv)
{
    ...
    testing::AddGlobalTestEnvironment(new life::ui::WxSession);   // GoogleTest owns it
