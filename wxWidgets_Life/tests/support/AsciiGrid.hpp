...
/// Builds a grid from rows of 'O' (alive) and '.' (dead), top row first: gridFromAscii({".O.", "..O"}).
...
[[nodiscard]] inline core::Grid gridFromAscii(std::initializer_list<std::string_view> rows)
{
    ...
    core::Grid grid({static_cast<core::Coord>(width), static_cast<core::Coord>(rows.size())});
    core::Coord y = 0;
    for (const std::string_view row : rows) {
        ...
        for (std::size_t x = 0; x < width; ++x) {
            ...
            grid.set({static_cast<core::Coord>(x), y}, row[x] == 'O' ? core::kAlive : core::kDead);
        }
        ++y;
    }
    return grid;
