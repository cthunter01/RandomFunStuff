...
// Restarts the pacer, then ticks every `interval` for `duration`, stepping everything each plan allows.
// Returns the generations stepped.
...
{
    GenerationPacer pacer(Speed{10, false});
...
{
    GenerationPacer pacer(Speed{250, false});
    pacer.restart(kStart);
    std::int64_t total = 0;
...
{
    GenerationPacer pacer(Speed{100, false});
    pacer.restart(kStart);
...
TEST(PacerTest, UnpaidDebtStaysCapped)
{
    ...
    GenerationPacer pacer(Speed{1000, false});
    pacer.restart(kStart);
...
{
    GenerationPacer pacer(Speed{30, true});
    pacer.restart(kStart);
...
{
    const std::vector<double> rates = measuredRates(Speed{1, false});
...
{
    const std::vector<double> rates = measuredRates(Speed{5, false});
    ASSERT_FALSE(rates.empty());
    for (const double rate : rates)
