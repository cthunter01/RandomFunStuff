...
// Replaces `grid` by its successor, after preparing the ghost border; returns the stepper's population.
...
Grid place(Pattern pattern, Extent extent, CellPos at)
{
    ...
    for (Coord y = 0; y < source.extent().height; ++y) {
        for (Coord x = 0; x < source.extent().width; ++x)
            grid.set({at.x + x, at.y + y}, source.at({x, y}));
    }
    return grid;
}
...
Grid randomGrid(Extent extent, std::uint64_t seed)
{
    ...
    for (Coord y = 0; y < extent.height; ++y) {
        for (Coord x = 0; x < extent.width; ++x)
            grid.set({x, y}, random() % 8 < 3 ? kAlive : kDead);
    }
    return grid;
}
...
{
    return {Rule{}, Rule::parse("B36/S23").value(), Rule::parse("B2/S").value(), Rule::parse("B0/S8").value(),
            Rule{0b1'0110'1001, 0b0'1001'0110}};
}
...
TEST_P(StepperEquivalenceTest, BandedMatchesReference)
{
    ...
    for (const unsigned threads : kMaxThreads)
        banded.push_back(
            std::make_unique<BandedStepper>(threads, 1));   // 1 cell per band: as many bands as rows
            ...
            for (int generation = 0; generation < param.generations; ++generation) {
                ...
                for (std::size_t i = 0; i < banded.size(); ++i) {
                    ...
                    ASSERT_EQ(actual, expected)
                        << "generation " << generation << ", maxThreads " << kMaxThreads[i];
...
TEST_P(StepperPatternTest, GliderMovesOneCellDiagonallyEveryFourGenerations)
{
    ...
    const Grid start = place(glider, {12, 10}, {1, 1});
    ...
    const Grid torusStart = place(glider, {6, 6}, {1, 1});
...
{
    const Grid start = place({".O.", "..O", "OOO"}, {6, 6}, {1, 1});
    const Grid block = gridFromAscii({
        "......",
        "......",
        "......",
...
TEST(StepperTest, EmptyGridsStepToEmptyGrids)
{
    for (const StepperKind kind : kStepperKinds) {
        ...
        for (const Extent extent : {Extent{0, 0}, Extent{0, 3}, Extent{3, 0}}) {
            Grid grid(extent);
