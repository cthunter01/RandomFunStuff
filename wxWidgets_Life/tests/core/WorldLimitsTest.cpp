...
static_assert(worldBytes({0, 0}) == 8);
static_assert(worldBytes({1, 1}) == 18);
static_assert(worldBytes({1000, 1000}) == 2 * 1002 * 1002);
static_assert(worldBytes({kMaxWorldSide, kMaxWorldSide}) == 2 * std::uint64_t{100'002} * 100'002);
...
{
    EXPECT_EQ(describe(ExtentError::TooSmall, {0, 5}, 4 * kGiB), "Width and height must be at least 1.");
