...
TEST(RuleTest, KernelMasksAgreeWithNextStateForEveryRule)
{
    ...
    for (unsigned birth = 0; birth <= 0x1FF; ++birth) {
        for (unsigned survival = 0; survival <= 0x1FF; ++survival) {
            ...
            for (unsigned n = 0; n <= 8; ++n) {
                // The kernel indexes by the 3×3 sum, which counts a live centre as one more neighbour.
                mismatches += ((masks.dead >> n) & 1u) != unsigned{rule.nextState(false, n)};
                mismatches += ((masks.alive >> (n + 1)) & 1u) != unsigned{rule.nextState(true, n)};
            }
        }
    }
...
TEST(RuleTest, FindPresetMatchesMasksNotText)
{
    const auto presetName = [](const Rule& rule) {
        ...
        return index ? kRulePresets[*index].name : std::string_view("none");
    };
    ...
    for (std::size_t i = 0; i < kRulePresets.size(); ++i)
        EXPECT_EQ(findPreset(kRulePresets[i].rule), i) << kRulePresets[i].name;
}
