...
bool borderWrapsAround(const Grid& grid)
{
    ...
    for (Coord y = -1; y <= height; ++y) {
        for (Coord x = -1; x <= width; ++x) {
            const bool ghost = !grid.extent().contains({x, y});
            if (ghost && padded(grid, x, y) != grid.at({wrap(x, width), wrap(y, height)}))
                return false;
        }
    }
    return true;
...
bool borderIsDead(const Grid& grid)
{
    ...
    for (Coord y = -1; y <= height; ++y) {
        for (Coord x = -1; x <= width; ++x) {
            if (!grid.extent().contains({x, y}) && padded(grid, x, y) != kDead)
                return false;
        }
    }
    return true;
...
{
    Grid grid({5, 4});
    grid.set({4, 3}, kAlive);   // bottom-right
    grid.updateBorder(Topology::Torus);
