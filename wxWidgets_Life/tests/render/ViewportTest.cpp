...
{
    const Extent world{static_cast<Coord>(pick(rng, 0, 60)), static_cast<Coord>(pick(rng, 0, 60))};
    const PixelSize canvas{pick(rng, 0, 400), pick(rng, 0, 300)};
    const int cellSize = static_cast<int>(pick(rng, kMinCellSize, kMaxCellSize));
    const PixelPoint offset{pick(rng, -50, Pixel{world.width} * cellSize + 50),
                            pick(rng, -50, Pixel{world.height} * cellSize + 50)};
...
WorldPoint worldAt(const Viewport& viewport, PixelPoint p)
{
    ...
    return {axis(viewport.offset().x, p.x), axis(viewport.offset().y, p.y)};
}
...
Span bruteForceVisible(Pixel offset, Pixel canvas, Coord side, int cellSize)
{
    ...
    return first ? Span{*first, end} : Span{};
}
...
std::optional<CellPos> bruteForceCellAt(const Viewport& viewport, PixelPoint p)
{
    ...
    return x && y ? std::optional(CellPos{*x, *y}) : std::nullopt;
}
...
TEST(ViewportTest, NearestZoomStep)
{
    ...
    for (std::size_t i = 0; i < kZoomSteps.size(); ++i)
        EXPECT_EQ(nearestZoomStep(kZoomSteps[i]), i);
}

TEST(ViewportTest, CellSizeIsClamped)
{
    Viewport viewport = makeViewport({10, 10}, {100, 100}, 4);
...
{
    const PixelSize canvas{640, 480};
    for (const int from : kZoomSteps) {
        // The canvas corners, and an anchor at each position inside a cell along x (y moves 3 px each time).
        std::vector anchors{PixelPoint{0, 0}, PixelPoint{639, 479}};
        for (Pixel d = 0; d < from; ++d)
            anchors.push_back({300 + d, 100 + 3 * d});
        for (const int to : kZoomSteps) {
            for (const PixelPoint anchor : anchors) {
                Viewport viewport = makeViewport({100'000, 100'000}, canvas, from);
                viewport.centerOn({50'000, 49'000});
...
TEST(ViewportTest, AnchoredZoomAlsoHoldsLeftOfTheWorld)
{
    ...
    Viewport viewport = makeViewport({10'000, 10'000}, {640, 480}, 4, {99, 0});
    const PixelPoint anchor{-500, 0};
...
TEST(ViewportTest, RepeatedZoomingDoesNotDrift)
{
    ...
    Viewport viewport = makeViewport({100'000, 100'000}, {800, 600}, 8);
    viewport.centerOn({31'415, 27'182});
    const PixelPoint anchor{313, 207};
    const WorldPoint point = worldAt(viewport, anchor);
    ...
    for (int i = 0; i < 2000; ++i) {
        ...
        const PixelPoint expected = offsets.try_emplace(viewport.cellSize(), viewport.offset()).first->second;
...
{
    // From every size and every anchor position inside a cell, zoom one step at a time to either end of
    // kZoomSteps and back, or jiggle between neighbours. The world is large, so nothing is clamped.
    const int last = static_cast<int>(kZoomSteps.size()) - 1;
    for (int start = 0; start <= last; ++start) {
        const int size = kZoomSteps[static_cast<std::size_t>(start)];
        for (Pixel d = 0; d < size; ++d) {
            const PixelPoint anchor{400 + d, 300 + d};
            // Each walk moves |walk| steps (in if positive), one step at a time.
            const auto offsetAfter = [&](const std::vector<int>& walks) {
                Viewport viewport = makeViewport({100'000, 100'000}, {800, 600}, size);
                viewport.centerOn({50'000, 50'000});
            ...
            const int neighbour = start > 0 ? -1 : 1;   // one step out and back in (at 1 px: in and out)
...
{
    // At 1 px the anchor pixel is a whole cell, so the kept point is the cell's centre. Step by step up to
    // 100 px, the anchor must end next to that centre (pixel 50 of 100), not in a corner of the cell.
    Viewport viewport = makeViewport({10'000, 10'000}, {800, 600}, 1);
    viewport.centerOn({5000, 5000});
    const PixelPoint anchor{537, 300};
    const std::optional<CellPos> cell = viewport.cellAt(anchor);
    ASSERT_TRUE(cell);
    while (viewport.cellSize() < kMaxCellSize)
        viewport.zoomBy(1, anchor);
    EXPECT_EQ(text(viewport.cellAt(anchor)), text(cell));
    EXPECT_EQ(text(viewport.cellOrigin(*cell)), text(PixelPoint{anchor.x - 50, anchor.y - 50}));
}

TEST(ViewportTest, ACentredAxisDoesNotEndTheOtherAxisRun)
{
    // A tall world in a wide canvas: x stays centred for the first zooms, and centring counts as clamping.
    // y is never clamped, so its run from 1 px must still end at the cell centre, as in the test above.
    Viewport viewport = makeViewport({100, 1000}, {2000, 1000}, 1);
    const PixelPoint anchor{1000, 500};
    const std::optional<CellPos> cell = viewport.cellAt(anchor);
    ...
    ASSERT_TRUE(now);
    EXPECT_EQ(now->y, cell->y);
    EXPECT_EQ(anchor.y - viewport.cellOrigin(*now).y, 50);
}

TEST(ViewportTest, OtherCameraChangesEndARunOfZooms)
{
    // After any other change, the next zoom keeps the cell that is under the anchor now, not the point
    // the run started on.
    const PixelPoint anchor{400, 300};
    const auto check = [&](std::string_view name, const auto& move) {
        Viewport viewport = makeViewport({20'000, 20'000}, {800, 600}, 10);
        viewport.centerOn({5000, 5000});
        viewport.zoomBy(1, anchor);
        move(viewport);
        const std::optional<CellPos> cell = viewport.cellAt(anchor);
    ...
    check("scrollTo", [](Viewport& v) { v.scrollTo({v.offset().x - 50, v.offset().y + 70}); });
    check("centerOn", [](Viewport& v) { v.centerOn({4990, 5010}); });
    check("fitWorld", [](Viewport& v) { v.fitWorld(); });
    check("setWorldExtent", [](Viewport& v) { v.setWorldExtent({5010, 5010}); });   // clamps the offset

    // A zoom that clamping moved ends the run too. At 80 px this one wants offset -90, which becomes 0.
    Viewport edge = makeViewport({10'000, 10'000}, {800, 600}, 100);
    const PixelPoint nearEdge{450, 300};
...
TEST(ViewportTest, SmallWorldIsCentred)
{
    ...
    Viewport viewport = makeViewport({10, 5}, {100, 50}, 4);
    ...
    viewport.setCanvasSize({101, 51});
    EXPECT_EQ(text(viewport.offset()), "(-30, -15)");

    // Exactly filling the canvas gives offset 0.
    viewport.setCanvasSize({40, 20});
    EXPECT_EQ(text(viewport.offset()), "(0, 0)");
}
...
TEST(ViewportTest, LargeWorldOffsetStaysInRange)
{
    ...
    Viewport viewport = makeViewport({1000, 1000}, {640, 480}, 4);
    viewport.scrollTo({-50, -1});
    EXPECT_EQ(text(viewport.offset()), "(0, 0)");
    viewport.scrollTo({1'000'000'000, 3520});
    EXPECT_EQ(text(viewport.offset()), "(3360, 3520)");
    viewport.scrollTo({100, 200});
...
TEST(ViewportTest, EachAxisIsClampedOnItsOwn)
{
    ...
    Viewport viewport = makeViewport({1000, 5}, {640, 480}, 4, {123, 456});
    EXPECT_EQ(text(viewport.offset()), "(123, -230)");
}

TEST(ViewportTest, ResizingReclampsTheOffset)
{
    Viewport viewport = makeViewport({1000, 1000}, {640, 480}, 4, {3360, 3520});

    viewport.setWorldExtent({500, 1000});   // content 2000 wide: offset.x may be at most 1360
    EXPECT_EQ(text(viewport.offset()), "(1360, 3520)");

    viewport.setCanvasSize({2100, 480});   // now the world is narrower than the canvas
    EXPECT_EQ(text(viewport.offset()), "(-50, 3520)");

    viewport.setCanvasSize({640, 480});   // the centred offset is clamped back into range
    EXPECT_EQ(text(viewport.offset()), "(0, 3520)");
}
...
TEST(ViewportTest, CellAtUsesFloorDivision)
{
    ...
    const Viewport viewport = makeViewport({10, 5}, {100, 50}, 4);
...
{
    const Viewport centred = makeViewport({10, 5}, {100, 50}, 4);
    ...
    const Viewport scrolled = makeViewport({1000, 1000}, {640, 480}, 4, {101, 202});
...
TEST(ViewportTest, CellAtMatchesBruteForce)
{
    ...
    for (int i = 0; i < 300; ++i) {
        ...
        for (int j = 0; j < 20; ++j) {
            const PixelPoint p{pick(rng, -120, 520), pick(rng, -120, 420)};
...
TEST(ViewportTest, CellOriginAndCellAtAreInverses)
{
    ...
    for (int i = 0; i < 300; ++i) {
        ...
        if (world.cellCount() == 0)
            continue;
        const int size = viewport.cellSize();
        const CellPos cell{static_cast<Coord>(pick(rng, 0, world.width - 1)),
                           static_cast<Coord>(pick(rng, 0, world.height - 1))};
        const PixelPoint origin = viewport.cellOrigin(cell);
        const PixelPoint last{origin.x + size - 1, origin.y + size - 1};
...
TEST(ViewportTest, VisibleCellsMatchBruteForce)
{
    ...
    for (int i = 0; i < 2000; ++i) {
        ...
        const Span ys =
            bruteForceVisible(viewport.offset().y, canvas.height, world.height, viewport.cellSize());
...
TEST(ViewportTest, FitWorldPicksTheLargestSizeThatShowsEverything)
{
    ...
    viewport.setWorldExtent({512, 512});
...
{
    Viewport viewport = makeViewport({5, 5}, {1000, 1000}, 4);
    viewport.fitWorld();
    EXPECT_EQ(viewport.cellSize(), kMaxCellSize);
...
TEST(ViewportTest, FitWorldCentresAWorldTooLargeToFit)
{
    ...
    Viewport viewport = makeViewport({20'000, 20'000}, {1000, 800}, 16);
    viewport.fitWorld();
    EXPECT_EQ(viewport.cellSize(), 1);
...
{
    Viewport viewport = makeViewport({1000, 1000}, {200, 100}, 10);
    viewport.centerOn({500, 500});
    EXPECT_EQ(text(viewport.offset()), "(4905, 4955)");   // 500 * 10 + 5 - canvas / 2
    EXPECT_EQ(text(viewport.cellAt({100, 50})), "(500, 500)");

    viewport.centerOn({0, 0});   // clamped at the top-left corner
    EXPECT_EQ(text(viewport.offset()), "(0, 0)");
    viewport.centerOn({999, 999});   // and at the bottom-right corner
    EXPECT_EQ(text(viewport.offset()), "(9800, 9900)");
}

TEST(ViewportTest, ZoomByWalksTheTable)
{
    Viewport viewport = makeViewport({100, 100}, {400, 400}, 4);
    viewport.zoomBy(1, {});
...
TEST(ViewportTest, ZoomByFromSizesNotInTheTable)
{
    ...
    const auto zoomedFrom = [](int cellSize, int steps) {
        Viewport viewport = makeViewport({100, 100}, {400, 400}, cellSize);
...
{
    Viewport viewport = makeViewport({10'000, 10'000}, {800, 600}, 7);
    viewport.centerOn({5000, 5000});
    const PixelPoint anchor{123, 456};
...
TEST(ViewportTest, LargestWorldAtLargestCellSize)
{
    ...
    Viewport viewport = makeViewport({side, side}, {1920, 1080}, kMaxCellSize, {kFar, kFar});
    ...
    EXPECT_EQ(text(viewport.cellAtClamped({1'000'000'000'000'000, -1'000'000'000'000'000})), "(99999, 0)");

    // Zooming out at the bottom-right corner keeps the last cell there.
    viewport.setCellSize(kMinCellSize, {1919, 1079});
...
TEST(ViewportTest, NoOverflowForTheLargestCoordinates)
{
    ...
    Viewport viewport = makeViewport({kMax, kMax}, {1920, 1080}, kMaxCellSize);
    viewport.scrollTo({std::numeric_limits<Pixel>::max(), std::numeric_limits<Pixel>::max()});
    ...
    viewport.centerOn({kMax - 100, 0});
    EXPECT_EQ(text(viewport.cellAt({960, 0})), std::format("({}, 0)", kMax - 100));
}

TEST(ViewportTest, EmptyCanvasShowsNothing)
{
    for (const PixelSize canvas : {PixelSize{0, 0}, PixelSize{0, 100}, PixelSize{100, 0}}) {
        const Viewport viewport = makeViewport({10, 10}, canvas, 4);
...
{
    const Viewport viewport = makeViewport({10, 10}, {-5, -7}, 4);
