...
public:
    ...
    void setSpeed(Speed speed) noexcept;   ///< Also drops accumulated debt.
    [[nodiscard]] Speed speed() const noexcept;
    void restart(Clock::time_point now) noexcept;   ///< Call when the simulation starts running.
    ...
    void commit(std::int64_t generationsDone) noexcept;   ///< Subtracts the generations actually stepped.
