...
TickPlan GenerationPacer::plan(Clock::time_point now) noexcept
{
    ...
    last_ = now;
    if (speed_.unlimited)
        return {std::numeric_limits<std::int64_t>::max(), kTickBudget};

    // The cap drops debt after a stall (a modal dialog, a slow step), so no burst of catch-up steps follows.
    // It also lets the rate settle at what the machine manages when ticks run out of time.
    ...
    return {static_cast<std::int64_t>(owed_), kTickBudget};   // whole generations only
}
...
{
    // A tick that stepped nothing is not a generation boundary, so it must not open or close a window.
    if (generations <= 0)
        return;
    if (!windowStart_) {   // the first batch's steps started at an unknown time, so they are not counted
        windowStart_ = now;
        return;
    }
