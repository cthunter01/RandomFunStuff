...
/// Dense cell storage padded by one ghost cell on every side, so neighbour sums need no edge checks.
/// Interior cell (x, y) is stored at padded position (x + 1, y + 1).
/// A moved-from Grid may only be assigned to or destroyed.
class Grid {
public:
    ...
    void set(CellPos p, Cell value) noexcept;          ///< @pre extent().contains(p), value is 0 or 1

    [[nodiscard]] std::span<const Cell> row(Coord y) const noexcept;   ///< Interior cells of row y.
    ...
    /// Rewrites the ghost border: zeros (Bounded) or copies of the opposite edges (Torus). O(width + height).
    void updateBorder(Topology topology) noexcept;
    void clear() noexcept;   ///< Every cell, border included, becomes dead.
