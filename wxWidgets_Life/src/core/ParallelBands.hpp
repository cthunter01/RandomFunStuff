...
/// covering [0, rows); `bands` is clamped to [1, max(rows, 1)]. Band 0 runs on the calling thread and
/// the others on std::jthreads; returns when all have finished. A band whose thread cannot be started
/// (std::system_error, std::bad_alloc) runs on the calling thread.
