...
/// Calls fn for every cell of the 8-connected line from a to b, both ends included, starting with a.
/// Swapping a and b visits the same cells in reverse order.
...
constexpr void forEachCellOnLine(CellPos a, CellPos b, Fn&& fn)
{
    ...
    const auto magnitude = [](std::int64_t v) { return v < 0 ? -v : v; };   // std::abs is not constexpr yet
    ...
    // After i steps the minor offset is floor(minor * i / steps + 1/2). Rounding halves the same way
    // in absolute coordinates is what makes both walking directions produce the same cells.
    // q and r are the quotient and remainder of (2 * minor * i + steps) / (2 * steps), kept incrementally
    // so that no product of two distances is ever formed.
    std::int64_t q = 0;
    std::int64_t r = steps;
    for (std::int64_t i = 0; i <= steps; ++i) {
        ...
        fn(CellPos{static_cast<Coord>(a.x + offsetX), static_cast<Coord>(a.y + offsetY)});
