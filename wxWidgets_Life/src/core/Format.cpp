...
std::string formatBytes(std::uint64_t bytes)
{
    ...
    static constexpr std::array<std::string_view, 6> kUnits{"KiB", "MiB", "GiB", "TiB", "PiB", "EiB"};
    ...
    return value < 99.95 ? std::format("{:.1f} {}", value, kUnits[unit])
                         : std::format("{:.0f} {}", value, kUnits[unit]);
}
