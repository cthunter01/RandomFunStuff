...
/// Ants a world may carry. Enough to watch them collide, and few enough that one generation stays trivial.
...
/// The cell one step ahead. It always wraps, whatever the world's Topology: an ant that walked off a
/// bounded edge would have to be deleted, while a wrapped one keeps drawing.
...
[[nodiscard]] constexpr CellPos forward(CellPos p, Heading h, Extent extent) noexcept
{
    switch (h) {
    case Heading::North:
        return {p.x, p.y > 0 ? p.y - 1 : extent.height - 1};
    case Heading::East:
        return {p.x + 1 < extent.width ? p.x + 1 : 0, p.y};
    case Heading::South:
        return {p.x, p.y + 1 < extent.height ? p.y + 1 : 0};
    case Heading::West:
        return {p.x > 0 ? p.x - 1 : extent.width - 1, p.y};
    }
    std::unreachable();
}
...
[[nodiscard]] constexpr Ant defaultAnt(int index, int count, Extent extent) noexcept
{
    ...
    return {.position = {x, extent.height / 2}, .heading = Heading::North};
}

/// One move: on a dead cell the ant turns right, on a live one left; then it flips the cell and steps
/// forward, wrapping at every edge.
