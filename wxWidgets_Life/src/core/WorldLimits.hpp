...
enum class ExtentError : std::uint8_t { TooSmall, TooLarge, OverMemoryBudget };
...
{
    return 2u * (static_cast<std::uint64_t>(e.width) + 2u) * (static_cast<std::uint64_t>(e.height) + 2u) *
           sizeof(Cell);
}
...
[[nodiscard]] std::expected<Extent, ExtentError> validateExtent(Extent e,
