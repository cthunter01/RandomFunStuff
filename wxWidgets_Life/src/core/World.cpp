...
// Every row has its own generator, seeded from the row number, so the band split cannot change the result.
// The row seed is hashed first. SplitMix64 advances its state by the same constant, so for small seeds the
// unhashed row seeds lie on one shared sequence and the rows come out as shifted copies of each other.
...
void World::randomize(double density, std::uint64_t seed)
{
    ...
    const auto threshold = static_cast<unsigned>(std::lround(std::clamp(density, 0.0, 1.0) * 256.0));
