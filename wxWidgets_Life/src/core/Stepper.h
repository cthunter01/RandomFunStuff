...
enum class StepperKind : std::uint8_t { Banded, Reference };
...
public:
    ...
    /// @pre src's ghost border matches `topology` (Grid::updateBorder); dst.extent() == src.extent().
...
/// @param maxThreads Only the Banded engine uses it: at most this many threads, 0 = hardware concurrency.
