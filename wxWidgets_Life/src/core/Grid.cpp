...
Grid::Grid(Extent extent)
    : extent_(extent), stride_(static_cast<std::size_t>(extent.width) + 2),
...
void Grid::updateBorder(Topology topology) noexcept
{
    ...
    case Topology::Bounded:
        ...
        break;
    case Topology::Torus:
        // Wrap the columns of every interior row first; the whole-row copies below then carry those
        // ghost cells along, which is what makes the four corners correct.
        for (std::size_t y = 1; y <= height; ++y) {
            const std::span<Cell> padded = paddedRowAt(y);
            padded[0] = padded[width];       // column -1 = column width - 1
            padded[width + 1] = padded[1];   // column width = column 0
        }
        std::ranges::copy(paddedRowAt(height), paddedRowAt(0).begin());       // row -1 = row height - 1
        std::ranges::copy(paddedRowAt(1), paddedRowAt(height + 1).begin());   // row height = row 0
        break;
    }
}
