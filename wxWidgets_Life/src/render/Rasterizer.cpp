...
// The bytes of pixels [from, to) in a run of RGB pixels. A whole frame is one long run, row after row.
...
void fillPixels(Bytes run, Rgb color) noexcept
{
    ...
    for (std::size_t filled = kPixelBytes; filled < run.size(); filled *= 2)
        std::ranges::copy(run.first(std::min(filled, run.size() - filled)), run.subspan(filled).begin());
}
...
struct Layout {
    ...
    [[nodiscard]] Pixel cellLeft(core::Coord cx) const noexcept { return Pixel{cx} * cellSize - offset.x; }
    [[nodiscard]] Pixel cellTop(core::Coord cy) const noexcept { return Pixel{cy} * cellSize - offset.y; }
...
// A horizontal grid-line row: `line` across the world, `majorLine` where a major vertical line crosses it.
void buildGridRow(std::vector<std::uint8_t>& row, const Layout& layout, Rgb line, Rgb majorLine, Rgb outside)
{
...
// Cell stamps indexed by [major vertical line][alive].
...
void drawScanline(Bytes scan, std::span<const core::Cell> cellRow, const Layout& layout, const Stamps& stamps,
                  const RenderStyle& style)
{
    ...
    const auto visible = cellRow.subspan(static_cast<std::size_t>(layout.cells.x0),
                                         static_cast<std::size_t>(layout.cells.x1 - layout.cells.x0));

    if (layout.cellSize == 1) {
        // 1 px cells: one palette lookup per pixel. (With grid lines, such cells have no body rows.)
        const std::array palette{style.dead, style.alive};
        ...
        for (const core::Cell cell : visible) {
            const Rgb color = palette[cell != core::kDead];
            *pixel++ = color.r;
            *pixel++ = color.g;
            *pixel++ = color.b;
        }
    ...
    for (const core::Cell cell : visible) {
        const ConstBytes stamp = stamps[layout.majorLineAfter(cx)][cell != core::kDead];
        ...
        std::ranges::copy(pixels(stamp, from - cellLeft, to - cellLeft), pixels(scan, from, to).begin());
        ++cx;
    }
}
                        ...
                        PixelBuffer& out)
{
    ...
    fillPixels(pixels(out.bytes(), layout.bottom * width, layout.canvas.height * width), style.outside);
    ...
    if (layout.gridLines) {
        ...
        buildGridRow(gridRowMajor_, layout, style.gridLineMajor, style.gridLineMajor, style.outside);
    }
    ...
    for (core::Coord cy = layout.cells.y0; cy < layout.cells.y1; ++cy) {
        ...
        const Pixel bodyEnd = std::min(layout.gridLines ? lineRow : lineRow + 1, layout.canvas.height);

        // Paint the first visible body row, then copy it; skipped when only the line row is visible.
        if (first < bodyEnd) {
        ...
        if (layout.gridLines && lineRow < layout.canvas.height)
            std::ranges::copy(layout.majorLineAfter(cy) ? gridRowMajor_ : gridRow_, out.row(lineRow).begin());
    }
}
