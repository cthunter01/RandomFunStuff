...
{
    canvas_ = {std::max<Pixel>(canvas.width, 0), std::max<Pixel>(canvas.height, 0)};
    clampOffset();
}
...
{
    return {Pixel{world_.width} * cellSize_, Pixel{world_.height} * cellSize_};
}
...
void Viewport::setCellSize(int px, PixelPoint anchor) noexcept
{
    ...
    const PixelPoint wanted{zoomedOffset(runX, offset_.x, anchor.x, cellSize_, px),
                            zoomedOffset(runY, offset_.y, anchor.y, cellSize_, px)};
    offset_ = wanted;
    cellSize_ = px;
    clampOffset();

    // An axis that clamping moved has lost its point, so its run ends; the other axis keeps its run.
                             ...
                             int px) noexcept
{
    ...
    // Integers only. The centre of content pixel c at cell size s lies (2c + 1) / 2s cells into the content,
    // so at size px it falls into content pixel ⌊(2c + 1) × px / 2s⌋, which goes under the anchor. That
    // pixel is in the same cell as c. Floor division, because the anchor may be left of the world.
...
void Viewport::zoomBy(int steps, PixelPoint anchor) noexcept
{
    ...
    const std::ptrdiff_t firstNotBelow = std::ranges::lower_bound(kZoomSteps, cellSize_) - kZoomSteps.begin();
    const std::ptrdiff_t firstAbove = std::ranges::upper_bound(kZoomSteps, cellSize_) - kZoomSteps.begin();
    ...
    setCellSize(kZoomSteps[static_cast<std::size_t>(index)], anchor);
}
...
void Viewport::fitWorld() noexcept
{
    ...
    const Pixel size = std::min(fits(canvas_.width, world_.width), fits(canvas_.height, world_.height));
    cellSize_ = static_cast<int>(std::clamp(size, Pixel{kMinCellSize}, Pixel{kMaxCellSize}));
    const PixelSize content = contentSize();
    offset_ = {(content.width - canvas_.width) / 2, (content.height - canvas_.height) / 2};
    clampOffset();
}
...
void Viewport::centerOn(core::CellPos cell) noexcept
{
    ...
    offset_ = {axis(cell.x, canvas_.width), axis(cell.y, canvas_.height)};
    clampOffset();
}
...
std::optional<core::CellPos> Viewport::cellAt(PixelPoint canvasPoint) const noexcept
{
    ...
    if (x < 0 || y < 0 || x >= world_.width || y >= world_.height)
        return std::nullopt;
    return core::CellPos{static_cast<core::Coord>(x), static_cast<core::Coord>(y)};
}
...
core::CellPos Viewport::cellAtClamped(PixelPoint canvasPoint) const noexcept
{
    ...
    return {axis(offset_.x + canvasPoint.x, world_.width), axis(offset_.y + canvasPoint.y, world_.height)};
}

PixelPoint Viewport::cellOrigin(core::CellPos cell) const noexcept
{
    return {Pixel{cell.x} * cellSize_ - offset_.x, Pixel{cell.y} * cellSize_ - offset_.y};
}
...
void Viewport::clampOffset() noexcept
{
    ...
    const PixelSize content = contentSize();
    offset_ = {axis(offset_.x, content.width, canvas_.width),
               axis(offset_.y, content.height, canvas_.height)};
}
