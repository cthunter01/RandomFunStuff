...
/// MainFrame::bindCommands() is the only table that maps commands to handlers. Camera moves and strokes
/// stay inside WorldCanvas, and Quit and About use wx's stock ids.
/// A plain enum on purpose: wx takes window ids as int.
enum CommandId : int {
...
/// Sends `id` as a wxEVT_MENU command from `source`; like every command event it propagates up to MainFrame.
