...
/// After each tick the timer is re-armed with at least kMinIdleGap of idle time: a GLib timeout that is
/// always ready starves GTK's lower-priority redraws (see docs/architecture.md).
...
public:
    ...
    void start();
    void stop();   ///< Safe when already stopped.
    void toggle();
    [[nodiscard]] bool isRunning() const noexcept;
    void stepOnce();   ///< Pauses first if running, then steps one generation.
    void setSpeed(core::Speed speed);
    [[nodiscard]] core::Speed speed() const noexcept;
    /// Generations per second over the last window of at least 0.5 s; nullopt until the first measurement
    /// after start().
