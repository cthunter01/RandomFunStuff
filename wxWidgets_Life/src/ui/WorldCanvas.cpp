...
static_assert(std::int64_t{core::kMaxWorldSide} * render::kMaxCellSize <= std::numeric_limits<int>::max());

namespace {

constexpr long kCanvasStyle =
    wxHSCROLL | wxVSCROLL | wxALWAYS_SHOW_SB | wxWANTS_CHARS | wxFULL_REPAINT_ON_RESIZE | wxBORDER_NONE;

constexpr int kWheelPanLines = 3;         // a wheel notch scrolls this many "lines"
constexpr int kMinWheelLinePx = 16;       // a line is one cell, but at least this many pixels
constexpr int kMinScrollbarLinePx = 32;   // same for the scrollbar arrows
...
WorldCanvas::WorldCanvas(wxWindow* parent, const core::World& world, Callbacks callbacks)
    : wxWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, kCanvasStyle), world_(world),
      callbacks_(std::move(callbacks)), style_(themeStyle())
{
    ...
    for (const auto& type : {wxEVT_LEFT_DOWN, wxEVT_LEFT_DCLICK, wxEVT_LEFT_UP, wxEVT_RIGHT_DOWN,
                             wxEVT_RIGHT_DCLICK, wxEVT_RIGHT_UP, wxEVT_MIDDLE_DOWN, wxEVT_MIDDLE_DCLICK,
                             wxEVT_MIDDLE_UP, wxEVT_MOTION, wxEVT_LEAVE_WINDOW})
        Bind(type, &WorldCanvas::onMouse, this);
    Bind(wxEVT_MOUSEWHEEL, &WorldCanvas::onWheel, this);
    for (const auto& type : {wxEVT_SCROLLWIN_TOP, wxEVT_SCROLLWIN_BOTTOM, wxEVT_SCROLLWIN_LINEUP,
...
{
    // The fit is kept while the canvas size changes: wx reports provisional sizes before and just after
    // Show(), and under Wayland the display scale can still change after the first frame.
    keepFitted_ = true;
...
void WorldCanvas::centerWorld()
{
    ...
    viewport_.scrollTo({(content.width - canvas.width) / 2, (content.height - canvas.height) / 2});
    viewportChanged();
}
...
core::Extent WorldCanvas::cellsThatFit() const noexcept
{
    ...
    return {cellsAlong(canvas.width), cellsAlong(canvas.height)};
}
...
void WorldCanvas::onPaint(wxPaintEvent&)
{
    wxPaintDC dc(this);
    wxASSERT(viewport_.worldExtent() == world_.extent());
    // The client area can change without a size event, for example with the display scale. Only the
    // viewport changes inside a paint handler; the scrollbars and the listeners follow right after it.
    if (syncCanvasSize())
    ...
    const wxImage image(static_cast<int>(width), static_cast<int>(height), frame_.bytes().data(), true);
    // Carrying the scale factor makes wx draw the bitmap at logical size, i.e. 1:1 on the physical screen.
...
void WorldCanvas::onMouse(wxMouseEvent& event)
{
    ...
    if (event.GetEventType() == wxEVT_LEAVE_WINDOW) {
        ...
        // wxGTK sends the second press of a double click only as a DCLICK, so that counts as a press too.
...
void WorldCanvas::onWheel(wxMouseEvent& event)
{
    ...
    // Smooth-scrolling devices send fractions of a notch; the leftovers are kept for the next event.
    ...
    if (event.ControlDown()) {
        ...
        const double pixels = notches * kWheelPanLines * std::max(viewport_.cellSize(), kMinWheelLinePx) *
                              (horizontalAxis ? 1 : -1);
...
void WorldCanvas::onScroll(wxScrollWinEvent& event)
{
    ...
    const auto along = [horizontal](render::PixelSize size) { return horizontal ? size.width : size.height; };
    ...
    const auto moveTo = [&](render::Pixel position) {
        viewport_.scrollTo(horizontal ? render::PixelPoint{position, offset.y}
                                      : render::PixelPoint{offset.x, position});
    };
...
void WorldCanvas::onKeyDown(wxKeyEvent& event)
{
    ...
    // GTK's scrolled window around the canvas binds Ctrl+Home and would take it from the menu accelerator.
    if (event.GetModifiers() == wxMOD_CONTROL && event.GetKeyCode() == WXK_HOME) {
        send(ID_CENTER_VIEW);
        return;
    }
    // Ctrl and Alt combinations belong to the menu accelerators (Shift does not count as a modifier here).
...
// Shortcuts that name a character are matched here: wxGTK reports punctuation keys in key-down events as a
// US layout would. On a German layout ']' is AltGr+9, which onKeyDown() sees as Ctrl+Alt+'9'.
void WorldCanvas::onChar(wxKeyEvent& event)
{
    // wx reports AltGr as Ctrl+Alt. Other Ctrl and Alt combinations belong to the menu accelerators.
...
void WorldCanvas::onCaptureLost(wxMouseCaptureLostEvent&)
{
...
void WorldCanvas::continuePaint(render::PixelPoint devicePoint)
{
    ...
    if (target == *lastStrokeCell_)
        return;
    // Painting the whole line from the previous cell leaves no gaps however fast the pointer moves.
    strokeCells_.clear();
    core::forEachCellOnLine(*lastStrokeCell_, target, [this](core::CellPos c) { strokeCells_.push_back(c); });
    lastStrokeCell_ = target;
...
void WorldCanvas::endDrag()
{
    ...
    // wx asserts when a window is destroyed while it holds the capture, or when a capture is released twice.
    if (HasCapture())
        ReleaseMouse();
}
...
void WorldCanvas::viewportChanged()
{
    ...
    // A stroke goes on from the cell now under the pointer. A line from the last cell would cross cells the
...
void WorldCanvas::syncScrollbars()
{
    const auto setBar = [this](int orientation, render::Pixel offset, render::Pixel canvas,
        ...
        // A centred world has a negative offset; the scrollbar then rests at 0. The range is at least one
        // page, because GTK shades a smaller range as if it were scrolled past its end.
...
render::PixelSize WorldCanvas::deviceClientSize() const
{
    ...
    return {static_cast<render::Pixel>(std::llround(size.x * scale)),
            static_cast<render::Pixel>(std::llround(size.y * scale))};
}

render::PixelPoint WorldCanvas::toDevice(wxPoint logical) const
{
    const double scale = GetContentScaleFactor();
    return {static_cast<render::Pixel>(std::llround(logical.x * scale)),
            static_cast<render::Pixel>(std::llround(logical.y * scale))};
}
