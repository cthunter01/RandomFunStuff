...
public:
    ...
    [[nodiscard]] static std::optional<WorldSizeRequest>
    ask(wxWindow* parent, core::Extent current, core::Extent fitsCanvas, std::uint64_t memoryBudgetBytes);

    /// wx asks this before it accepts the dialog. It matters for Enter in a size box: wxGTK then presses
    /// OK even while it is disabled.
    bool Validate() override;

private:
    WorldSizeDialog(wxWindow* parent, core::Extent current, core::Extent fitsCanvas,
                    std::uint64_t memoryBudgetBytes);
    [[nodiscard]] std::optional<WorldSizeRequest> request() const;   ///< nullopt for an invalid size.
    ...
    void revalidate();   ///< Updates the memory and error lines; enables OK only for valid sizes.
