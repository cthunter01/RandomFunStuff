    ...
    : wxScrolledWindow(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVSCROLL | wxTAB_TRAVERSAL)
{
    ...
    addRuleGroup(*column);
    SetSizer(column);
    SetScrollRate(0, FromDIP(kScrollStepDip));   // vertical only, so short screens can reach every group
...
void ControlPanel::setSpeed(core::Speed speed)
{
    ...
    // Several slider positions share one rate; leave the slider where the user put it if it already fits.
...
void ControlPanel::setRule(const core::Rule& rule)
{
    ...
    rulePreset_->SetSelection(static_cast<int>(preset.value_or(core::kRulePresets.size())));   // or "Custom"
    setRuleError({});
}
...
void ControlPanel::setRuleError(std::string_view message)
{
    if (message.empty() && !ruleError_->IsShown())
        return;
    ruleError_->SetLabelText(toWx(message));   // SetLabelText: an '&' is shown, not taken as a mnemonic
...
void ControlPanel::addViewGroup(wxSizer& column)
{
    ...
    cellSizeSlider_->Bind(wxEVT_SLIDER, [this](wxCommandEvent&) {
        cellSizeSpin_->SetValue(render::kZoomSteps[static_cast<std::size_t>(cellSizeSlider_->GetValue())]);
        emitCommand(*cellSizeSlider_, ID_CELL_SIZE_CHANGED);
    });
    cellSizeSpin_->Bind(wxEVT_SPINCTRL, [this](wxSpinEvent&) {
        cellSizeSlider_->SetValue(static_cast<int>(render::nearestZoomStep(cellSizeSpin_->GetValue())));
...
void ControlPanel::addRuleGroup(wxSizer& column)
{
    ...
    ruleText_ =
        new wxTextCtrl(box, wxID_ANY, wxString(), wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER);
    auto* apply = new wxButton(box, wxID_ANY, "Apply");
    ruleError_ = new wxStaticText(box, wxID_ANY, wxString());
    ruleText_->SetToolTip("B/S notation: B = neighbour counts that give birth, S = counts that survive");
