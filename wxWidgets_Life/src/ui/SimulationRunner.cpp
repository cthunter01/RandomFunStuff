...
void SimulationRunner::onTimer()
{
    ...
    while (done < plan.maxGenerations && (done == 0 || core::Clock::now() - start < plan.timeBudget)) {
        world_.step();
        ++done;
    }
    pacer_.commit(done);
...
// One-shot on purpose: a repeating GLib timeout whose handler outlasts its interval is always ready,
// and GTK, which repaints at a lower priority, then never repaints. The minimum gap leaves time for
// input and paint events after every tick.
void SimulationRunner::scheduleNext(core::Clock::duration tickCost)
{
    using namespace std::chrono;
    const milliseconds delay = std::max(kMinIdleGap, kTickInterval - duration_cast<milliseconds>(tickCost));
