...
constexpr std::array<core::Coord, 8> kSquareSides{100, 256, 500, 1'000, 2'000, 5'000, 10'000, 20'000};
...
{
    return new wxSpinCtrl(parent, wxID_ANY, wxString(), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS,
                          core::kMinWorldSide, core::kMaxWorldSide, value);
}

// The number typed into a side box, or nullopt if the text is not a whole number. wxSpinCtrl::GetValue()
// would silently clamp it to the spin range instead. Numbers beyond the limits stay beyond them, so
// validateExtent() rejects them.
[[nodiscard]] std::optional<core::Coord> typedSide(const wxSpinCtrl& spin)
{
    ...
    if (digits.starts_with('+') && !digits.starts_with("+-"))   // GTK's box accepts '+'; from_chars does not
...
WorldSizeDialog::WorldSizeDialog(wxWindow* parent, core::Extent current, core::Extent fitsCanvas,
    ...
    : wxDialog(parent, wxID_ANY, "World Size"), fitsCanvas_(fitsCanvas), budget_(memoryBudgetBytes)
{
    ...
    for (const core::Coord side : kSquareSides)
        presetNames.Add(toWx(sizeText({side, side})));
    ...
    memory_ = new wxStaticText(this, wxID_ANY,
                               toWx(memoryText(core::formatBytes(core::worldBytes(largest)), budget_)));
    const std::array<std::string, 4> errors{
        std::string(kNotWholeNumbers), core::describe(core::ExtentError::TooSmall, largest, budget_),
    ...
    presets_->Bind(wxEVT_CHOICE, [this](wxCommandEvent& event) { applyPreset(event.GetSelection()); });
    revalidate();
}
...
std::expected<core::Extent, std::string> WorldSizeDialog::typedExtent() const
{
    ...
    const core::Extent extent{*width, *height};
...
void WorldSizeDialog::applyPreset(int index)
{
    ...
    if (index < kFitWindowIndex) {
        const core::Coord side = kSquareSides[static_cast<std::size_t>(index - kFirstSquareIndex)];
        extent = {side, side};
    }
